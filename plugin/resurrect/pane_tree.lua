...
local function insert_panes(root, panes)
	...
	if not wezterm.mux.get_domain(domain):is_spawnable() then
		...
		if domain == "local" then
			...
			-- pty from a short-lived alt-screen program (e.g. `man`'s pager) while
			-- is_alt_screen_active() still reads true from a moment earlier. Both
			-- fall through to text capture rather than persisting a bogus/missing
			-- process or crashing.
			if
				process_info
				and not utils.COMMON_SHELLS[utils.base_name_of(process_info.name or process_info.executable or "")]
			then
				...
				-- replayed baseline byte-identical; an active pane's capture
				-- has its trailing live prompt block stripped when the
				-- measured exemplar vouches for it. Either way the persisted
				-- text never ends in a prompt the next restore's fresh shell
				-- would duplicate, so save->restore cycles cannot grow it.
...
function pub.default_on_pane_restore(pane_tree)
	...
	if pane_tree.alt_screen_active and pane_tree.process and pane_tree.process.argv then
		local base_name = utils.base_name_of(pane_tree.process.name)
